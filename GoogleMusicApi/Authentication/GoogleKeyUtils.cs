...
namespace GoogleMusicApi.Authentication
{
    ...
    internal class GoogleKeyUtils
    {
        ...
        {
            var rsa = WinRTCrypto.AsymmetricKeyAlgorithmProvider.OpenAlgorithm(AsymmetricAlgorithm.RsaOaepSha512);
            var rsaKey = rsa.ImportParameters(key);

            var sha1 = WinRTCrypto.HashAlgorithmProvider.OpenAlgorithm(HashAlgorithm.Sha1);

            byte[] prefix = { 0x00 };

            var hash = sha1.HashData(KeyToStruct(key)).Take(4).ToArray();

            var encrypted = WinRTCrypto.CryptographicEngine.Encrypt(rsaKey, Encoding.UTF8.GetBytes(email + "\x00" + password), null);
